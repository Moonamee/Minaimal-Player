  ...
  try {
    ...
    if (soundObj === null) {
      ...
      updateState(context, {
        ...
        activePlayList: [],
        ...playListInfo,
        bitrate: audio.bitrate, // Установите значение битрейта в контекст
        sampleRate: audio.sampleRate, // Установите значение частоты дискретизации в контекст
      });
...
//-------------------FOR RANDOM-----------------------------------
export const randomAudio = async (context, select) => {
  const {
    playbackObj,
    currentAudioIndex,
    totalAudioCount,
  ...
  if (isPlayListRunning) {
    return selectAudioFromPlayList(context, select);
  }

  try {
    const { isLoaded } = await playbackObj.getStatusAsync();
    const randomIndex = Math.floor(Math.random() * totalAudioCount);
    ...
    console.log("error inside random audio method.", error.message);
  }
};
